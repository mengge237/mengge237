# 👋 你好，我是 mengge237

**计算机科学与技术 · 本科在读 · 2027届** · 游戏开发 / 全栈 / AI 应用方向

> 独立开发 Unity 卡牌 Roguelike 游戏原型、Java 全栈学习平台、AI 联机游戏平台；热爱游戏、建模与全栈开发。

## 📊 技术栈

| 方向 | 技能 |
|---|---|
| 🎮 游戏开发 | Unity / C# / ScriptableObject / URP / Spine / Blender |
| ☕ 后端 | Java / Spring Boot / JWT / JPA / MySQL / Node.js / Express / WebSocket |
| 🖥️ 前端 | Vue3 / Three.js / GSAP / HTML5 / CSS3 / JavaScript |
| 🤖 AI | Semantic Kernel / Ollama / LLM 集成（智谱 / DeepSeek / OpenAI） |
| 🛠 其他 | Python / Git / MCP for Unity / RESTful API |

## 🚀 项目作品集

### 🎮 游戏
| 项目 | 说明 | 链接 |
|---|---|---|
| **异变棋局（畸变骑士）** | Unity 卡牌 Roguelike 游戏原型：程序化地图/回合战斗/卡组构筑/遗物合成，57+ commits，已出 Windows 可运行版 | [GitHub](https://github.com/mengge237/distortion-knight-development) |
| **Blender 作品集** | 模型/场景/材质/几何节点/特效 38+ 件练习作品 | [GitHub](https://github.com/mengge237/blender-works) |

### ☕ Java 全栈
| 项目 | 说明 | 链接 |
|---|---|---|
| **AI 智学 LearnAI** | Spring Boot 4 + Vue3 + Three.js 学习平台：AI 答疑/学习路径/3D 模型商城（17 张表） | [GitHub](https://github.com/mengge237/learnai) |
| **AI Agent 联机游戏平台** | Spring Boot + WebSocket + LLM：谁是卧底等 5 玩法，多模型切换/TTS/断线重连 | [后端](https://github.com/mengge237/ai-agent-platform) · [前端](https://github.com/mengge237/ai-undercover-agent) |

### 🖥️ 前端 / Web
| 项目 | 说明 | 链接 |
|---|---|---|
| **Three.js 3D Demo** | 3D 场景/动画/GLTF 加载（Vite 工程） | [GitHub](https://github.com/mengge237/threejs-demos) |
| **前端交互 Demo 集** | 50+ 独立交互页面：3D/粒子/动效/响应式 | [GitHub](https://github.com/mengge237/html-interactive-demos) |
| **个人作品集站** | 个人博客 + 作品集展示 | [GitHub](https://github.com/mengge237/personal-portfolio) |

### 🔧 工具 / 其他
| 项目 | 说明 | 链接 |
|---|---|---|
| **记账本后端** | Express + MySQL + JWT 多模块 API | [GitHub](https://github.com/mengge237/accounting-app-backend) |
| **路径算法可视化** | Python/Pygame 五种路径算法 + 性能对比 | [GitHub](https://github.com/mengge237/maze-pathfinding-visualizer) |
| **书签整理工具** | Python + Chrome 扩展批量管理书签 | [GitHub](https://github.com/mengge237/bookmark-organizer) |
| **AI 对话 Demo** | C# Semantic Kernel + Ollama 本地 LLM | [GitHub](https://github.com/mengge237/semantic-kernel-ollama-demo) |

## 📫 联系

- GitHub：https://github.com/mengge237
- 欢迎交流游戏开发、全栈与 AI 应用方向的技术与项目
